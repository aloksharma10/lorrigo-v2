...
import { Button, DropdownMenu, DropdownMenuContent, DropdownMenuRadioGroup, DropdownMenuRadioItem, DropdownMenuTrigger } from './';
...
const SplitButton = forwardRef<HTMLDivElement, SplitButtonProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn('divide-primary-foreground/30 inline-flex -space-x-px divide-x rounded-lg shadow-sm shadow-black/5 rtl:space-x-reverse', className)}
        {...props}
      >
        <Button
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronDown className="h-4 w-4" strokeWidth={2} aria-hidden="true" />}
            </Button>
          </DropdownMenuTrigger>
            ...
            <DropdownMenuRadioGroup value={selectedValue} onValueChange={handleValueChange}>
              {options.map((option) => (
                <DropdownMenuRadioItem
                  ...
                  className={cn('items-start [&>span]:pt-1.5', option.disabled && 'cursor-not-allowed opacity-50')}
                >
