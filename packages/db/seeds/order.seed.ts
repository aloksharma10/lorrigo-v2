      ...
      const result = await prisma.$transaction(async (tx) => {
         ...
         const order = await tx.order.create({
            data: {
               ...
               user_id: "cmc3mmkmt0000h02g3j29wurh",
               customer_id: customer.id,
         ...
         const shipment = await tx.shipment.create({
            data: {
               ...
               order_id: order.id,
               user_id: "cmc3mmkmt0000h02g3j29wurh",
               courier_id: "cmbq7bjz0009nh0ikk24yll18",
               created_at: new Date(mongoData.createdAt.$date),
               updated_at: new Date(mongoData.updatedAt.$date),
            },
         });
         ...
         return { order, customer, shipment, trackingEvents, orderItem, sellerDetails, customerAddress, packageData, orderChannelConfig };
      });
