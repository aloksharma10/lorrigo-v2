...
export function CSVUploadModal() {
  ...
  if (open && minimized && step === 'processing') {
    return (
      <div className="bg-background fixed bottom-4 right-4 z-50 w-64 rounded-lg border p-4 shadow-lg">
                ...
                {EXPECTED_HEADERS.map((expectedHeader) => (
                  <div key={expectedHeader} className="grid grid-cols-2 items-center gap-4">
                    ...
                    <Popover
                      ...
                      onOpenChange={(open) =>
                        setOpenComboboxes((prev) => ({ ...prev, [expectedHeader]: open }))
                      }
                    >
                              ...
                              {csvHeaders.map((header) => (
                                ...
                                >
                                  <Check
                                    className={cn(
                                      'mr-2 h-4 w-4',
                                      headerMapping[expectedHeader] === header
                                        ? 'opacity-100'
                                        : 'opacity-0'
                                    )}
                                  />
