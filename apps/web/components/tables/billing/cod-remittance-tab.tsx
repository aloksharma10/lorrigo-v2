...
export default function CODRemittanceTab() {
  ...
  const columns: ColumnDef<CODRemittance>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <Info className="h-4 w-4 text-gray-400" />₹ {amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}
          </div>
        )
      },
      enableSorting: true,
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="Amount Credited To Wallet" />,
      cell: ({ row }) => {
