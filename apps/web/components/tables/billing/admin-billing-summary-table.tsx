...
export function AdminBillingSummaryTable({ month, onUserSelect, onManualBilling, onBillingCycle }: AdminBillingSummaryTableProps) {
  const [pagination, setPagination] = useState({
    pageIndex: 0,
    pageSize: 15,
  });
  const [sorting, setSorting] = useState<{ id: string; desc: boolean }[]>([{ id: 'total_billing_amount', desc: true }]);
  ...
  const columns: ColumnDef<BillingSummaryByUser>[] = [
        ...
        return (
          <div className="flex flex-col">
            ...
            <div className="text-muted-foreground text-sm">
              <CopyBtn label={user.user_email} tooltipText="Copy Email" text={user.user_email} labelClassName="hover:underline" />
            </div>
            <div className="text-muted-foreground text-xs">
              <CopyBtn label={`ID: ${user.user_id}`} tooltipText="Copy User ID" text={user.user_id} />
            </div>
          </div>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-center">
            <div className="text-lg font-bold text-blue-600">{user.total_orders}</div>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-right">
            <div className="text-lg font-bold text-green-600">{currencyFormatter(user.total_billing_amount)}</div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const user = row.original;
        const paidPercentage = user.total_billing_amount > 0 ? (user.paid_amount / user.total_billing_amount) * 100 : 0;
        ...
        return (
          <div className="space-y-2">
            ...
            <div className="mt-2">
              {paidPercentage === 100 ? (
                <Badge className="border-green-200 bg-green-100 text-green-800">Fully Paid</Badge>
              ) : paidPercentage > 80 ? (
                <Badge className="border-blue-200 bg-blue-100 text-blue-800">Mostly Paid</Badge>
              ) : paidPercentage > 0 ? (
                <Badge className="border-orange-200 bg-orange-100 text-orange-800">Partially Paid</Badge>
              ) : (
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => onUserSelect(user.user_id, user.user_name)} className="h-8 gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem onClick={() => onUserSelect(user.user_id, user.user_name)} className="gap-2">
                  <Eye className="h-4 w-4" />
                  View Billing Details
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                {onManualBilling && (
                  <DropdownMenuItem onClick={() => onManualBilling(user.user_id, user.user_name)} className="gap-2">
                    <Calculator className="h-4 w-4" />
                    Manual Billing
                  </DropdownMenuItem>
                )}
                {onBillingCycle && (
                  <DropdownMenuItem onClick={() => onBillingCycle(user.user_id, user.user_name)} className="gap-2">
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-1 text-sm">There was an error loading the billing summary. Please try again.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-muted-foreground text-xs">Users with billing records</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">{summaryStats.paidPercentage.toFixed(1)}%</div>
              <p className="text-muted-foreground text-xs">{currencyFormatter(summaryStats.totalPaid)} paid</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{currencyFormatter(summaryStats.totalPending)}</div>
              <p className="text-muted-foreground text-xs">{summaryStats.pendingPercentage.toFixed(1)}% of total</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">{currencyFormatter(summaryStats.avgOrderValue)}</div>
              <p className="text-muted-foreground text-xs">Across {summaryStats.totalOrders} orders</p>
            </CardContent>
          </Card>
        </div>
      )}
