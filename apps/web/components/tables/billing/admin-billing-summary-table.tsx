...
export function AdminBillingSummaryTable({ 
  ...
  const columns: ColumnDef<BillingSummaryByUser>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const user = row.original;
        const paidPercentage = user.total_billing_amount > 0 
          ? (user.paid_amount / user.total_billing_amount) * 100 
          : 0;
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-bold text-green-600">
                {currencyFormatter(user.paid_amount)}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-orange-600 font-medium">Pending:</span>
              <span className="font-bold text-orange-600">
                {currencyFormatter(user.pending_amount)}
              </span>
            </div>
            {user.disputed_amount > 0 && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-red-600">
                  {currencyFormatter(user.disputed_amount)}
                </span>
              </div>
            )}
  ...
  // Filterable columns for better filtering capabilities
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {currencyFormatter(summaryStats.totalPaid)} paid
              </p>
            </CardContent>
          </Card>
