...
export class RemittanceService {
   ...
   * This is the main function that runs daily via BullMQ
   */
  async calculateRemittanceForAllUsers() {
    console.log('Starting daily remittance calculation...');
    
    try {
      // Find all eligible users with their bank accounts, wallet, and profile
      const users = await this.fastify.prisma.user.findMany({
        ...
        include: { 
          profile: true,
          wallet: true,
          UserBankAccount: true // Include bank accounts
        },
      });
  ...
  private async processUserRemittance(user: any) {
    ...
    // Check if user has more than 10 bank accounts
    if (user.UserBankAccount.length > 10) {
      console.log(`User ${user.id} has more than 10 bank accounts, skipping`);
      return;
    }

    // Find eligible bank accounts (verified and older than 72 hours)
    const eligibleBankAccounts = user.UserBankAccount.filter((acc: any) => 
      this.isBankAccountEligible(acc)
    );
    ...
    // Get user-selected bank account for remittance, or fallback to latest eligible
    const selectedBankAccount = eligibleBankAccounts.find((acc: any) => acc.is_selected_for_remittance) 
      || eligibleBankAccounts[eligibleBankAccounts.length - 1];
    ...
    // Get remittance configuration from user profile
    const remittanceDelay = user.profile?.remittance_days_after_delivery || 7; // Default D+7
    const remittanceDays = user.profile?.remittance_days_of_week || [5]; // Default Friday
    ...
    const ordersGroupedByRemittanceDate = this.groupOrdersByRemittanceDate(
      eligibleOrders,
      remittanceDelay,
      remittanceDays
    );
    ...
    const cutoffDate = new Date('2025-06-10'); // Fixed cutoff for June 10, 2025

    // Process remittances for each remittance date
    for (const [remittanceDateStr, ordersForRemittance] of Object.entries(ordersGroupedByRemittanceDate)) {
      ...
      // Skip past or pre-cutoff remittance dates
      ...
      const existingRemittance = await this.fastify.prisma.remittance.findFirst({
        where: {
          user_id: user.id,
          remittance_date: {
            gte: startOfDay(remittanceDate),
            lte: startOfDay(remittanceDate),
          },
        },
      });
      ...
      if (existingRemittance) {
        ...
        await this.createRemittanceRecord(
          user,
          selectedBankAccount,
          ordersForRemittance,
          remittanceCalculation,
  ...
  private async findEligibleOrders(userId: string, remittanceDelay: number) {
    ...
    return await this.fastify.prisma.order.findMany({
      where: {
        user_id: userId,
        remittanceId: null, // Not already remitted
        shipment: { 
          status: 'DELIVERED',
          updated_at: { lte: cutoffDate }, // Orders delivered before cutoff
        },
        payment_method: 'COD', // Only COD orders
      },
      include: {
        shipment: true,
        billings: true,
  ...
  private findNearestRemittanceDay(date: Date, remittanceDays: number[]): Date {
    ...
    // Fallback to next Friday if no match found
  ...
  private async calculateRemittanceAmount(orders: any[], earlyRemittanceChargeRate: number) {
    ...
    for (const order of orders) {
      ...
      // Calculate charges from billings
  ...
  private async createRemittanceRecord(user: any, bankAccount: any, orders: any[], calculation: any, remittanceDate: Date) {
    return await this.fastify.prisma.$transaction(async (tx) => {
      ...
      if (user.wallet) {
        ...
        // Update remittance with wallet details
   ...
   */
  async getRemittancesForUser(userId: string) {
    const remittances = await this.fastify.prisma.remittance.findMany({
  ...
  async getRemittanceById(id: string) {
    ...
    if (!remittance) {
      return { valid: false, message: 'No Remittance found' };
    }
   ...
   */
  async getAllRemittances({ page = 1, limit = 20, status, userId, from, to, search }: any) {
  ...
  /**
   * Get future remittances (admin)
   */
  async getFutureRemittances({ page = 1, limit = 20, userId }: any) {
    const skip = (page - 1) * limit;
    const where: any = { remittance_date: { gte: new Date() } };
  ...
  async getRemittanceAnalytics({ userId, period }: any) {
    ...
    const where: any = { 
      remittance_date: { gte: from, lte: new Date() } 
    };
  ...
  async getUserBankAccounts(userId: string) {
    const bankAccounts = await this.fastify.prisma.userBankAccount.findMany({
      where: { user_id: userId },
      orderBy: { created_at: 'desc' },
    });
    
    return { valid: true, bankAccounts };
  }
  ...
  async addBankAccount(userId: string, bankAccountData: any) {
    ...
    const existingCount = await this.fastify.prisma.userBankAccount.count({
      where: { user_id: userId },
    });
  ...
  async selectBankAccountForRemittance(userId: string, bankAccountId: string) {
    // Verify the bank account belongs to the user and is eligible
    ...
    // Update all bank accounts to unselected, then select the chosen one
  ...
  async verifyBankAccount(bankAccountId: string, isVerified: boolean, adminId: string) {
    const bankAccount = await this.fastify.prisma.userBankAccount.findUnique({
      where: { id: bankAccountId },
    });
  ...
  async exportRemittancesAsCSV(filters: any) {
    // Get all remittances with filters (no pagination for export)
  ...
  async exportRemittanceDetailAsCSV(id: string) {
    const { remittanceOrder } = await this.getRemittanceById(id);
    if (!remittanceOrder) {
