...
export function initShipmentAnalysisQueue(fastify: FastifyInstance) {
  ...
  [homeAnalyticsWorker, performanceAnalyticsWorker, realTimeAnalyticsWorker, predictiveAnalyticsWorker, cacheUpdateWorker, reportGenerationWorker].forEach(
    (worker) => {
...
export async function addShipmentAnalysisJob(
  ...
  return await addJob(QueueNames.REPORT_GENERATION, jobType, data, {
